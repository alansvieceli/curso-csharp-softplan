...
namespace Microservice.Whatever.Repositories.Abstractions
{
public abstract class Repository<TEntity, TId> : IRepository<TEntity, TId>
        where TEntity : EntityBase<TId>
        where TId : struct
    {
        private readonly DbContext _context; //Banco de dados
        private readonly DbSet<TEntity> _dbSet; // Tabela
        ...
        {
            if (await ExistsAsync(entity.Id, cancellationToken).ConfigureAwait(false)) 
                return;
        ...
        {
            if (await ExistsAsync(entity.Id, cancellationToken).ConfigureAwait(false) == false) 
                return;
